...
static void ParseTensor(std::shared_ptr<DNNTensor> cls_tensor, 
  ...
  if (ret != 0) {
    ...
    int cls_batch = cls_tensor->properties.validShape.dimensionSize[0];
    int bbox_batch = bbox_tensor->properties.validShape.dimensionSize[0];
    if (batch_index < 0 || batch_index >= cls_batch || batch_index >= bbox_batch) {
    RCLCPP_ERROR(rclcpp::get_logger("Yolo11_detection_parser"),
           "Invalid batch index %d, cls_batch=%d, bbox_batch=%d",
           batch_index,
           cls_batch,
           bbox_batch);
    return;
    ...
    size_t cls_batch_offset =
      static_cast<size_t>(batch_index) * height * width * CLASSES_NUM;
    size_t bbox_batch_offset =
      static_cast<size_t>(batch_index) * height * width * REG * 4;
